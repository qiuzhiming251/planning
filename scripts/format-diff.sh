...
if [ "$CLANG_FORMAT_DIFF" ]; then
    ...
    elif clang-format-diff-14 --help &>/dev/null </dev/null; then
        CLANG_FORMAT_DIFF=clang-format-diff-14
    elif clang-format-diff-10 --help &>/dev/null </dev/null; then
        CLANG_FORMAT_DIFF=clang-format-diff-10
    else
...
if [ -z "$uncommitted_code" ]; then
    ...
    # Use FETCH_HEAD branch from that remote
    [ "$FORMAT_UPSTREAM" ] || FORMAT_UPSTREAM="FETCH_HEAD"
