#include "plan_common/math/quartic_polynomial_curve1d.h"
...
std::string QuarticPolynomialCurve1d::ToString() const {
  std::string log_str;
  log_str = absl::StrCat("v:", end_condition_[0], " a:", end_condition_[1],
                         " t:", param_);
  return log_str;
}
...
bool QuarticPolynomialCurve1d::GetParams(double& s1, double& s2, double& s3,
                                         double& p) const {
  s1 = 0;
  s2 = end_condition_[0];
  s3 = end_condition_[1];
