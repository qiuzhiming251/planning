...
namespace ad_byd {
namespace planning {
bool CurveBoundary1d::Create(const LineCurve1d &lower_boundary,
                             const LineCurve1d &upper_boundary,
                             double s_interval) {
  boundary_map_.clear();
  if (!lower_boundary.IsValid() && !upper_boundary.IsValid()) {
    LINFO("No valid boundary, create CurveBoundary1d fail!");
    return false;
  }
  ...
  while (s < s_end + Constants::ZERO) {
    ...
    boundary_map_[s] = std::move(std::make_pair(lower_bound, upper_bound));
    s += s_interval;
  }
  return true;
}

void CurveBoundary1d::SetBoundary(const double x, const double lower,
                                  const double upper) {
  if (lower < upper) {
    // remove equal key
    ...
    boundary_map_[x] = std::make_pair(lower, upper);
  }
}

bool CurveBoundary1d::GetBoundary(const double x, double *lower,
                                  double *upper) const {
  ...
  if (boundary_map_.size() == 1) {
    if (lower) {
      *lower = boundary_map_.begin()->second.first;
    }
    if (upper) {
      *upper = boundary_map_.begin()->second.second;
    }
    return true;
  }
  ...
  if (it0 == boundary_map_.begin()) {
    ...
    if (lower) {
      *lower = boundary_map_.rbegin()->second.first;
    }
    if (upper) {
      *upper = boundary_map_.rbegin()->second.second;
    }
    return true;
  }
  auto it1 = it0;
  it0--;
  if (lower) {
    *lower = math::lerp(it0->second.first, it0->first, it1->second.first,
                        it1->first, x, true);
  }
  if (upper) {
    *upper = math::lerp(it0->second.second, it0->first, it1->second.second,
                        it1->first, x, true);
  }
  return true;
}
bool CurveBoundary1d::GetBoundary(std::vector<Point2d> &lower,
  ...
  for (const auto &it : boundary_map_) {
    lower.emplace_back(it.first, it.second.first);
    upper.emplace_back(it.first, it.second.second);
  }
  return true;
}

bool CurveBoundary1d::GetRange(double *begin, double *end) const {
  if (boundary_map_.empty()) {
    return false;
  }
  if (begin) {
