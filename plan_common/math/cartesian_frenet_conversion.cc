...
void CartesianFrenetConverter::cartesian_to_frenet(
  ...
  if (fabs(fabs(delta_theta) - M_PI / 2) < 0.01) {
    // std::cout << "delta_theta equal to 90 degress, cartesian_to_frenet fail!"
    // << std::endl;
  }
  ...
  ptr_d_condition->at(1) =
      one_minus_kappa_r_d * tan_delta_theta;  // tan(dtheta)
...
void CartesianFrenetConverter::frenet_to_cartesian(
  ...
  *ptr_y = ry + cos_theta_r * d_condition[0];
  ...
  const double delta_theta =
      st::fast_math::Atan2(d_condition[1], one_minus_kappa_r_d);
  const double cos_delta_theta = std::cos(delta_theta);
  ...
  *ptr_a = s_condition[2] * one_minus_kappa_r_d / cos_delta_theta +
  ...
  //     LOG_INFO <<
  //     "frenet_to_cartesian:"<<*ptr_theta<<"||"<<rtheta<<"||"<<rkappa<<"||"<<d_condition[0]<<"||"
  //     <<d_condition[1]<<"||"<<s_condition[1];
}
...
double CartesianFrenetConverter::CalculateTheta(const double rtheta,
                                                ...
                                                const double l,
                                                const double dl) {
  return NormalizeAngle(rtheta + st::fast_math::Atan2(dl, 1 - l * rkappa));
}
...
double CartesianFrenetConverter::CalculateLateralDerivative(
    ...
    const double rkappa) {
  return (1 - rkappa * l) * math::tan(math::Angle16::FromRad(theta - rtheta));
}
...
double CartesianFrenetConverter::CalculateSecondOrderLateralDerivative(
  ...
  const double res = -(rdkappa * l + rkappa * dl) * std::tan(theta_diff) +
  ...
  if (std::isinf(res)) {
    std::cout << "result is inf when calculate second order lateral "
              << "derivative. input values are rtheta:" << rtheta
              << " theta: " << theta << ", rkappa: " << rkappa
