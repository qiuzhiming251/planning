...
class LineSearchMoreThuente {
 private:
  ...
  static Scalar quadratic_interp(const Scalar& a, const Scalar& b,
  ...
  // Assume that a != b
  static Scalar cubic_interp(const Scalar& a, const Scalar& b, const Scalar& fa,
                             const Scalar& fb, const Scalar& ga,
                             const Scalar& gb) {
    using std::abs;
    using std::sqrt;
    ...
    if (abs(z) < std::numeric_limits<Scalar>::epsilon()) {
      ...
      // If c2 <= 0, or globmin is outside [a, b], then the minimum is achieved
      // at one end point
  ...
  static Scalar step_selection(const Scalar& al, const Scalar& au,
    ...
    // Case 1: ft > fl
    if (ft > fl)
      return (std::abs(ac - al) < std::abs(aq - al)) ? ac
                                                     : ((aq + ac) / Scalar(2));
 ...
 public:
  ...
  /// length. \param step_max The upper bound for the step size. \param drt The
  /// current moving direction. \param xp       The current point. \param param
  /// Parameters for the LBFGS algorithm
  ///
    ...
    for (iter = 0; iter < param.max_linesearch; iter++) {
      ...
      if (step < param.min_step)
        throw std::runtime_error(
            "the line search step became smaller than the minimum value "
            "allowed");
