...
#include "plan_common/math/geometry/LBFGSpp/BFGSMat.h"
...
//
// Class to compute the generalized Cauchy point (GCP) for the L-BFGS-B
...
// After passing a break point, one or more coordinates of x will be fixed at
// the bounds. We search the first local minimum of m(x) by examining the
// intervals [t[i-1], t[i]] sequentially.
//
// Reference:
// [1] R. H. Byrd, P. Lu, and J. Nocedal (1995). A limited memory algorithm for
...
class Cauchy {
 private:
  ...
  // Find the smallest index i such that brk[ord[i]] > t, assuming brk[ord] is
  // already sorted. If the return value equals n, then all values are <= t.
  static int search_greater(const Vector& brk, const IndexSet& ord,
                            const Scalar& t, int start = 0) {
 ...
 public:
  ...
  static void get_cauchy_point(const BFGSMat<Scalar, true>& bfgs,
                               const Vector& x0, const Vector& g,
                               const Vector& lb, const Vector& ub, Vector& xcp,
                               Vector& vecc, IndexSet& newact_set,
                               IndexSet& fv_set) {
    ...
    /* int iter = 0;
    std::cout << "** Iter " << iter << " **\n";
    std::cout << "   fp = " << fp << ", fpp = " << fpp << ", deltatmin = " <<
    deltatmin << std::endl; std::cout << "   il = " << il << ", iu = " << iu <<
    ", deltat = " << deltat << std::endl; */

    // If deltatmin >= deltat, we need to do the following things:
    // 1. Update vecc
    // 2. Since we are going to cross iu, the coordinates that define iu become
    // active
    // 3. Update some quantities on these new active coordinates (xcp, vecd,
    // vecp)
    ...
    while (deltatmin >= deltat) {
      ...
      // Step 2
      // First check how many coordinates will be active when we cross the
      // previous iu b is the smallest number such that brko[b] == iu Let bp be
      // the largest number such that brko[bp] == iu Then coordinates ord[b] to
      // ord[bp] will be active
      const int act_begin = b;
      const int act_end = search_greater(brk, ord, iu, b) - 1;

      // If nfree == 0 and act_end == nord-1, then we have crossed all
      // coordinates We only need to update xcp from ord[b] to ord[bp], and then
      // exit
      ...
      il = iu;
      b = act_end + 1;
      // If we have visited all finite-valued break points, and have not exited
      // earlier, then the next iu will be infinity. Simply exit the loop now
      if (b >= nord) break;
      iu = brk[ord[b]];
