...
// 4. Ashcraft, C., Grimes, R. G., & Lewis, J. G. (1998). Accurate symmetric
// indefinite linear equation solvers.
...
class BKLDLT {
 private:
  ...
  IntVector m_perm;  // [-2, -1, 3, 1, 4, 5]: 0 <-> 2, 1 <-> 1, 2 <-> 3, 3 <->
                     // 1, 4 <-> 4, 5 <-> 5
  ...
  // Largest (in magnitude) off-diagonal element in the first column of the
  // current reduced matrix r is the row index Assume k < end
  ...
  // Largest (in magnitude) off-diagonal element in the r-th column of the
  // current reduced matrix p is the row index Assume k < r < end
      ...
      if (abs_akk < alpha * lambda) {
        ...
        if (sigma * abs_akk < alpha * lambda * lambda) {
          if (abs_akk >= alpha * sigma) {
            ...
            // Right now we use Version 1 since it reduces the overhead of
            // interchange
  ...
  int gaussian_elimination_1x1(Index k) {
    ...
    if (akk == Scalar(0)) return NUMERICAL_ISSUE;
  ...
  int gaussian_elimination_2x2(Index k) {
    ...
    // Return NUMERICAL_ISSUE if not invertible
    if (e11 * e22 - e21 * e21 == Scalar(0)) return NUMERICAL_ISSUE;
 ...
 public:
  ...
  void compute(ConstGenericMatrix& mat, int uplo = Eigen::Lower,
    ...
    if (k == m_n - 1) {
      const Scalar akk = diag_coeff(k);
      if (akk == Scalar(0)) m_info = NUMERICAL_ISSUE;
