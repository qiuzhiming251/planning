...
#include "plan_common/math/geometry/LBFGSpp/BKLDLT.h"
...
// large scale optimization. [2] R. H. Byrd, P. Lu, and J. Nocedal (1995). A
...
class BFGSMat {
 private:
  ...
  Scalar
      m_theta;  // theta * I is the initial approximation to the Hessian matrix
  ...
  int m_ptr;  // A Pointer to locate the most recent history, 1 <= m_ptr <= m
              ...
              //          For example, if the current s-vector is stored in
              //          m_s[, m-1], then in the next iteration m_s[, 0] will
              //          be overwritten. m_s[, m_ptr-1] points to the most
              //          recent history, and m_s[, m_ptr % m] points to the
              //          most distant one.
 ...
 public:
  ...
  inline void add_correction(const RefConstVec& s, const RefConstVec& y) {
    ...
    if (LBFGSB) {
      ...
      // Update S'S
      // We only store S'S in Minv, and multiply theta when LU decomposition is
      // performed
      ...
      if (m_ncorr >= m_m) m_permMinv.block(m_m, loc, m_m, 1).setZero();
  ...
  // Recursive formula to compute a * H * v, where a is a scalar, and v is [n x
  // 1] H0 = (1/theta) * I is the initial approximation to H Algorithm 7.4 of
  ...
  // If the P'W matrix has been explicitly formed, do a direct matrix
  // multiplication
  ...
  // Usually d contains many zeros, so we fist compute number of nonzero
  // elements in A set and F set, denoted as nnz_act and nnz_fv, respectively If
  // nnz_act is smaller, compute W'AA'd = WA' (A'd) directly If nnz_fv is
  // smaller, compute W'AA'd = W'd - WF' * (F'd)
  inline void compute_FtBAb(const Matrix& WF, const IndexSet& fv_set,
                            const IndexSet& newact_set, const Vector& Wd,
                            const Vector& drt, Vector& res) const {
  ...
  // inv(P'BP) * v = v / theta + WP * inv(inv(M) - WP' * WP / theta) * WP' * v /
  // theta^2
  //
  // v is [nP x 1]
  inline void solve_PtBP(const Matrix& WP, const Vector& v, Vector& res) const {
    ...
    // Compute the matrix in the middle (only the lower triangular part is
    // needed) Remember that W = [Y, theta * S], but we do not store theta in WP
  ...
  // Compute P'BQv, where P and Q are two mutually exclusive index selection
  // operators P'BQv = -WP * M * WQ' * v Returns false if the result is known to
  // be zero
  inline bool apply_PtBQv(const Matrix& WP, const IndexSet& Q_set,
  ...
  // If the Q'W matrix has been explicitly formed, do a direct matrix
  // multiplication
  inline bool apply_PtBQv(const Matrix& WP, const Matrix& WQ, const Vector& v,
                          Vector& res) const {
    const int nP = WP.rows();
    ...
    // Remember that W = [Y, theta * S], so we need to multiply theta to the
    // second half
