...
#include "plan_common/math/geometry/LBFGSpp/Cauchy.h"
#include "plan_common/math/geometry/LBFGSpp/LineSearchMoreThuente.h"
#include "plan_common/math/geometry/LBFGSpp/Param.h"
#include "plan_common/math/geometry/LBFGSpp/SubspaceMin.h"
...
class LBFGSBSolver {
 private:
  ...
  // ||P(x-g, l, u) - x||_inf
  static Scalar proj_grad_norm(const Vector& x, const Vector& g,
                               const Vector& lb, const Vector& ub) {
    return ((x - g).cwiseMax(lb).cwiseMin(ub) - x).cwiseAbs().maxCoeff();
  }

  // The maximum step size alpha such that x0 + alpha * d stays within the
  // bounds
  static Scalar max_step_size(const Vector& x0, const Vector& drt,
                              const Vector& lb, const Vector& ub) {
    const int n = x0.size();
 ...
 public:
  ...
  ///
  /// Minimizing a multivariate function subject to box constraints, using the
  /// L-BFGS-B algorithm. Exceptions will be thrown if error occurs.
  ///
  ...
  inline int minimize(Foo& f, Vector& x, Scalar& fx, const Vector& lb,
                      const Vector& ub) {
    using std::abs;
    ...
    // std::cout << "f(x0) = " << fx << ", ||proj_grad|| = " << projgnorm <<
    // std::endl << std::endl;
    ...
    std::cout << "f(xcp) = " << fcp << ", ||proj_grad|| = " << projgcpnorm <<
    std::endl << std::endl; */
    ...
    for (;;) {
      ...
      LineSearch<Scalar>::LineSearch(f, fx, x, m_grad, step, step_max, m_drt,
                                     m_xp, m_param);
      ...
      std::cout << "   f(x) = " << fx << ", ||proj_grad|| = " << projgnorm <<
      std::endl << std::endl; */
      ...
      std::cout << "f(xsm) = " << fsm << ", ||proj_grad|| = " << projgsmnorm <<
      std::endl << std::endl;*/
