...
///
/// A line search algorithm for the strong Wolfe condition. Implementation based
/// on:
///
  ...
  static void LineSearch(Foo& f, Scalar& fx, Vector& x, Vector& grad,
    ...
    if (param.linesearch != LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE)
      throw std::invalid_argument(
          ...
          "'LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE' for "
      ...
      if (fx - fx_init > step * test_decr || fx >= fx_lo) {
        if (step == step_hi)
          throw std::runtime_error(
              "the line search routine failed, possibly due to insufficient "
              "numeric precision");
