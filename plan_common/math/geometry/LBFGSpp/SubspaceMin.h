...
//
// The target of subspace minimization is to minimize the quadratic function
// m(x) over the free variables, subject to the bound condition. Free variables
// stand for coordinates that are not at the boundary in xcp, the generalized
// Cauchy point.
//
// In the classical implementation of L-BFGS-B [1], the minimization is done by
// first ignoring the box constraints, followed by a line search. Our
// implementation is an exact minimization subject to the bounds, based on the
// BOXCQP algorithm [2].
//
// Reference:
// [1] R. H. Byrd, P. Lu, and J. Nocedal (1995). A limited memory algorithm for
// bound constrained optimization. [2] C. Voglis and I. E. Lagaris (2004).
...
class SubspaceMin {
 private:
  ...
  static bool P_converged(const IndexSet& yP_set, const Vector& vecy,
    ...
    for (int i = 0; i < nP; i++) {
      ...
      if (vecy[coord] < vecl[coord] || vecy[coord] > vecu[coord]) return false;
    }
    return true;
  }
 ...
 public:
  ...
  static void subspace_minimize(const BFGSMat<Scalar, true>& bfgs,
                                const Vector& x0, const Vector& xcp,
                                const Vector& g, const Vector& lb,
                                const Vector& ub, const Vector& Wd,
    ...
    if (nfree < 1) {
      // std::cout << "========================= (Early) leaving subspace
      // minimization =========================\n\n";
      return;
    }

    // std::cout << "New active set = [ "; for(std::size_t i = 0; i <
    // newact_set.size(); i++)  std::cout << newact_set[i] << " "; std::cout <<
    // "]\n"; std::cout << "Free variable set = [ "; for(std::size_t i = 0; i <
    // fv_set.size(); i++)  std::cout << fv_set[i] << " "; std::cout << "]\n\n";
    ...
    // Set the vector c=F'BAb+F'g for linear term, and vectors l and u for the
    // new bounds
    ...
    for (k = 0; k < maxit; k++) {
      ...
      /* std::cout << "** Iter " << k << " **\n";
      std::cout << "   L = [ "; for(std::size_t i = 0; i < L_set.size(); i++)
      std::cout << L_set[i] << " "; std::cout << "]\n"; std::cout << "   U = [
      "; for(std::size_t i = 0; i < U_set.size(); i++)  std::cout << U_set[i] <<
      " "; std::cout << "]\n"; std::cout << "   P = [ "; for(std::size_t i = 0;
      i < P_set.size(); i++)  std::cout << P_set[i] << " "; std::cout <<
      "]\n\n"; */
