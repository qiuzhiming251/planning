...
//   }
//   return Double::Compare(val, bound1) != CompareType::LESS &&
//   Double::Compare(val, bound2) != CompareType::GREATER;
// }
...
//   length_ = hypot(dx, dy);
//   unit_direction_ =
//       (Double::Compare(length_, 0.0) != CompareType::GREATER ? Vec2d(0, 0) :
//       Vec2d(dx / length_, dy / length_));
//   heading_ = unit_direction_.Angle();
// }

// void LineSegment2d::Reset(const Vec2d &start, const Vec2d &end) {
//   start_ = start;
//   end_ = end;
//   const double dx = end_.x() - start_.x();
//   const double dy = end_.y() - start_.y();
...
// Vec2d LineSegment2d::rotate_expand(const double angle, const double length)
// const {
//   Vec2d diff_vec = end_ - start_;
//   diff_vec.SelfRotate(angle);
//   diff_vec.Normalize();
...
//   if (Double::Compare(length_, 0.0) != CompareType::GREATER) {
...
// double LineSegment2d::DistanceTo(const Vec2d &point, Vec2d *const nearest_pt)
// const {
...
// double LineSegment2d::DistanceSquareTo(const Vec2d &point, Vec2d *const
// nearest_pt) const {
//   CHECK_NOTNULL(nearest_pt);
//   if (Double::Compare(length_, 0.0) != CompareType::GREATER) {
//     *nearest_pt = start_;
...
//     return false;
//   }
//   return IsWithin(point.x(), start_.x(), end_.x()) && IsWithin(point.y(),
//   start_.y(), end_.y());
// }

// double LineSegment2d::ProjectOntoUnit(const Vec2d &point) const { return
// unit_direction_.InnerProd(point - start_); }

// double LineSegment2d::ProductOntoUnit(const Vec2d &point) const { return
// unit_direction_.CrossProd(point - start_); }
...
// bool LineSegment2d::GetIntersect(const LineSegment2d &other_segment, Vec2d
// *const point) const {
//   CHECK_NOTNULL(point);
...
//   start_); const double cc4 = CrossProd(other_segment.start(),
//   other_segment.end(), end_); if (cc3 * cc4 >= -kMathEpsilon) {
//     return false;
//   }
//   const double ratio = cc4 / (cc4 - cc3);
//   *point = Vec2d(start_.x() * ratio + end_.x() * (1.0 - ratio), start_.y() *
//   ratio + end_.y() * (1.0 - ratio)); return true;
// }

// // return distance with perpendicular foot point.
// double LineSegment2d::GetPerpendicularFoot(const Vec2d &point, Vec2d *const
// foot_point) const {
