...
//                   [&](const LineSegment2d &poly_seg) { return
//                   poly_seg.HasIntersect(line_segment); })) {
//     return 0.0;
//   }

//   double distance = std::min(DistanceTo(line_segment.start()),
//   DistanceTo(line_segment.end())); for (int i = 0; i < num_points_; ++i) {
...
//     GetAllOverlaps(line_segment, &overlaps);
...
//                      [&](const LineSegment2d &line_segment) { return
//                      Contains(line_segment); });
// }

// int Polygon2d::Next(int at) const { return at >= num_points_ - 1 ? 0 : at +
// 1; }

// int Polygon2d::Prev(int at) const { return at == 0 ? num_points_ - 1 : at -
// 1; }
...
// bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points, Polygon2d
// *const polygon) {
//   if (!polygon) {
//     return false;
//   }
...
//     sorted_indices[i] = i;
//   }
//   std::sort(sorted_indices.begin(), sorted_indices.end(), [&](const int idx1,
//   const int idx2) {
...
//     (*points)[i]); if (std::abs(prod[i]) <= kMathEpsilon) {
//       side[i] = 0;
//     } else {
//       side[i] = ((prod[i] < 0) ? -1 : 1);
//     }
...
//       new_points.emplace_back((*points)[i].x() * ratio + (*points)[j].x() *
//       (1.0 - ratio),
//                               (*points)[i].y() * ratio + (*points)[j].y() *
//                               (1.0 - ratio));
//     }
//   }
...
// bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon, Polygon2d
// *const overlap_polygon) const {
//   if (points_.size() < 3) {
//     return 0.0;
//   }
...
//       || (line_segment.start().y() < min_y_ && line_segment.end().y() <
//       min_y_) || (line_segment.start().y() > max_y_ && line_segment.end().y()
//       > max_y_)) {
//     return false;
//   }
//   Vec2d first;
...
// bool Polygon2d::GetOverlap(const LineSegment2d &line_segment, Vec2d *const
// first, Vec2d *const last) const {
//   if (points_.size() < 3) {
//     return 0.0;
//   }
...
// void Polygon2d::GetAllOverlaps(const LineSegment2d &line_segment,
//                                std::vector<LineSegment2d> *const
//                                overlap_line_segments) const {
//   if (line_segment.length() <= kMathEpsilon) {
//     if (IsPointIn(line_segment.start())) {
//       overlap_line_segments->emplace_back(line_segment);
//     }
//     return;
//   }
...
//       continue;
//     }
//     const Vec2d reference_point = line_segment.start() + (start_proj +
//     end_proj) / 2.0 * line_segment.unit_direction(); if
//     (!IsPointIn(reference_point)) {
//       continue;
//     }
//     if (overlaps.empty() || start_proj > overlaps.back().second +
//     kMathEpsilon) {
...
//     overlap_line_segments->emplace_back(line_segment.start() + overlap.first
//     * line_segment.unit_direction(),
//                                         line_segment.start() + overlap.second
//                                         * line_segment.unit_direction());
//   }
//   return;
// }

// void Polygon2d::ExtremePoints(const double heading, Vec2d *const first, Vec2d
// *const last) const {
...
//   return Box2d((x1 + x2) / 2.0 * direction_vec + (y1 + y2) / 2.0 *
...
//       points.push_back(points_[i] + Vec2d::CreateUnitVec2d(start_angle) *
//       distance);
//     } else {
//       const int count = static_cast<int>(diff / kMinAngle) + 1;
//       for (int k = 0; k <= count; ++k) {
//         const double angle = start_angle + diff * static_cast<double>(k) /
//         static_cast<double>(count); points.push_back(points_[i] +
