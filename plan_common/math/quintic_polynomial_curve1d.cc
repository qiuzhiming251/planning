...
std::string QuinticPolynomialCurve1d::ToString() const {
  std::string log_str;
  log_str = absl::StrCat("s:", end_condition_[0], " v:", end_condition_[0],
                         " a:", end_condition_[1], " t:", param_);
  // LOG_INFO << log_str;
  return log_str;
}
...
bool QuinticPolynomialCurve1d::GetParams(double& s1, double& s2, double& s3,
                                         double& p) const {
  s1 = end_condition_[0], s2 = end_condition_[1], s3 = end_condition_[2];
  p = param_;
  return true;
}
