...
// namespace st::status_macro_internal {
...
class StatusAdaptorForMacros {
 public:
  StatusAdaptorForMacros(const absl::Status& status, const char* /* file */,
                         int /* line */)
      : builder_(status) {}

  StatusAdaptorForMacros(absl::Status&& status, const char* /* file */,
                         int /* line */)
      : builder_(status) {}
...
// Evaluates an expression that produces a `Status`. If the status
// is not ok, returns it from the current function.
//
// For example:
//   Status MultiStepFunction() {
//     RETURN_IF_ERROR(Function(args...));
//     RETURN_IF_ERROR(foo.Method(args...));
//     return absl::OkStatus();
//   }
//
// The macro ends with a `StatusBuilder` which allows the returned
...
//   }
//
// `StatusBuilder` supports adapting the builder chain using a
...
// details.
//
//   void Read(StringPiece name, Task* task) {
//     int64 id;
...
// to avoid confusion between a `StatusBuilder` and a
// `Status` type. E.g.
//
//   []() -> Status {
//     RETURN_IF_ERROR(Function(args...));
...
#define RETURN_IF_ERROR(expr)                                             \
  ...
  if (st::status_macro_internal::StatusAdaptorForMacros                   \
          status_macro_internal_adaptor = {(expr), __FILE__, __LINE__}) { \
  } else /* NOLINT */                                                     \
    return status_macro_internal_adaptor.Consume()

// Executes an expression `rexpr` that returns a `StatusOr<T>`. On
...
// well as a `StatusBuilder` object populated with the error and
...
#endif  // ST_PLANNING_UTIL_STATUS_MACROS
