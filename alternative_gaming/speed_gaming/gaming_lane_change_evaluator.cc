...
void GamingLaneChangeEvaluator::Init(
...
// Lane Change gaming evaluate
double GamingLaneChangeEvaluator::LaneChangeSafetyEvaluate(
...
double GamingLaneChangeEvaluator::GetWeight(size_t t) {
  if (t >= w_index_.size()) {
    return 0.0;
  }
  return w_index_[t];
...
double GamingLaneChangeEvaluator::CalculateAccSmoothness(
...
double GamingLaneChangeEvaluator::CalculateJerkSmoothness(
    GamingCostType cost_type) {
  const SpeedVector &speed_data = cost_type == GamingCostType::kEgo
                                      ? gaming_sim_result_.ego_speed_data
                                      : gaming_sim_result_.obj_speed_data;
...
double GamingLaneChangeEvaluator::CalculateSpeedEfficiency(
    GamingCostType cost_type) {
...
int GamingLaneChangeEvaluator::CheckBreakToStop(GamingCostType cost_type) {
...
double GamingLaneChangeEvaluator::CalculateTravelEfficiency(
    GamingCostType cost_type) {
  const SpeedVector &speed_data = cost_type == GamingCostType::kEgo
                                      ? gaming_sim_result_.ego_speed_data
                                      : gaming_sim_result_.obj_speed_data;
...
double GamingLaneChangeEvaluator::CalculateStaticSafetyCost(
    bool &is_collision_result) {
  ...
  is_collision_result = safe_stationary_cost > 0.333;

  return std::min(std::max(safe_stationary_cost, 0.0), 2.0);
}

double GamingLaneChangeEvaluator::CalculateMinBoxDistance(const size_t i) {
...
double GamingLaneChangeEvaluator::CalculatePointToSegmentDistance(
