...
VERSION_X86_64="dev-x86-3.1.4"
VERSION_ORIN_X86_64="dev-x86-3.1.4"
VERSION_ORIN_X2B_X86_64="dev-x86-2.4.0"
VERSION_J6M_X86_64="dev-x86-2.0.4"
TESTING_VERSION_X86_64="cyber-x86_64-18.04-testing-20210108_1510"

CYBER_CONTAINER="byd_ird_${USER}"
...
function parse_arguments() {
    ...
    while [[ $# -gt 0 ]]; do
        ...
        -n | --name)
            ...
            CYBER_INSIDE="$1"
            shift
            optarg_check_for_opt "${opt}" "${CYBER_CONTAINER}"
            ;;
        -h | --help)
            show_usage
            exit 1
        ...
        -p | --platform)
            PLATFORM_TYPE="$1"
            shift
            optarg_check_for_opt "${opt}" "${PLATFORM_TYPE}"
            ;;
        stop)
            local force="$1"
...
function determine_target_version_and_arch() {
    ...
    if [[ -z "${version}" ]]; then
        ...
        if [[ "${TARGET_ARCH}" == "x86_64" ]]; then
            if [[ "${PLATFORM_TYPE}" == "orin" ]]; then
                info "\033[33mPLATFORM_TYPE\033[0m type is orin"
                DOCKER_REPO="10.64.251.130:5000/byd-image/software/orin/byd_orin_n2"
                version="${VERSION_X86_64}"
            elif [[ "${PLATFORM_TYPE}" == "j6m" ]]; then
                info "\033[33mPLATFORM_TYPE\033[0m type is j6m"
                DOCKER_REPO="10.64.251.130:5000/byd-image/software/j6/byd_j6m"
                version=$VERSION_J6M_X86_64
            elif [[ "${PLATFORM_TYPE}" == "x2b" ]]; then
                info "\033[33mPLATFORM_TYPE\033[0m type is x2b"
                DOCKER_REPO="10.64.251.130:5000/byd-image/software/orin/byd_orin_x2b"
                version=$VERSION_ORIN_X2B_X86_64
            else
                error "\033[33mPLATFORM_TYPE\033[0m is not specified. The command needs to add -p j6m/orin/x2b"
                exit 1
            fi
            RUNTIME=" "
            if [[ $(nvidia-smi | grep "Driver Version" | wc -l) -lt 1 ]]; then
                echo "nvidia-smi not work proper, init with no gpu"
                GPUS=" "
            else
                GPUS=""
            fi
        ...
        if [[ -z "${supposed_arch}" ]]; then
            ...
            error "  Expected formattarget>-<arch>-<ubuntu_release>-<timestamp>"
            exit 1
        fi
        if [[ -z "${TARGET_ARCH}" ]]; then
...
function setup_devices_and_mount_volumes() {
    ...
    # bash rc
    local bashrc="${HOME}/.bashrc_docker"
    if [[ -f "${bashrc}" ]]; then
        chmod a+rw "${bashrc}"
        volumes+=" --mount type=bind,source=${bashrc},target=${HOME}/.personal_bashrc"
    fi

    # vscode-server
    local vscode_server_dir="${HOME}/.vscode-server"
    if [[ -d "${vscode_server_dir}" ]]; then
        volumes+=" --mount type=bind,source=${vscode_server_dir},target=${HOME}/.vscode-server"
    fi

    # .gitconfig
    local gitconfig="${HOME}/.gitconfig"
    if [[ -f "${gitconfig}" ]]; then
        volumes+=" --mount type=bind,source=${gitconfig},target=${HOME}/.gitconfig"
    ...
    # .ssh
    if [[ -f ${HOME}/.ssh ]]; then
        volumes+=" -v ${HOME}/.ssh:${HOME}/.ssh"
    fi

    # docker should check
    # local dockercmd="/usr/bin/docker"
    # if [[ -f  "${dockercmd}" ]]; then
    #     volumes+=" --mount type=bind,source="${dockercmd}",target=${dockercmd}"
    # fi

    # oss_util
    local oss_util="/usr/bin/ossutil"
    if [[ -f "${oss_util}" ]]; then
        volumes+=" -v $oss_util:$oss_util"
    fi

    if $(grep -q WSL2 /proc/version); then
        #    volumes+=" -v /mnt/c:/mnt/c"
        volumes+=" -v /mnt/d:/mnt/d"
    fi

    #docker in docker
    volumes+=" -v /var/run/docker.sock:/var/run/docker.sock:rw"
    volumes+=" -v /lib/systemd/system:/lib/systemd/system:rw"
    volumes+=" -v /etc/network:/etc/network:rw"
    volumes+=" -v /etc/netplan:/etc/netplan:rw"
    volumes+=" -v ${HOME}:/hosthome"
...
function postrun_bashrc() {
    local container="$1"
    if [[ -f "${HOME}/.bashrc_docker" ]]; then
        docker exec -u "${USER}" "${container}" \
            bash -c "echo 'source ~/.personal_bashrc && source /usr/share/bash-completion/completions/git' >> ~/.bashrc"
    fi
}
...
function main() {
    ...
    postrun_bashrc "${CYBER_CONTAINER}"
